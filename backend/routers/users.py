...
def update_user_data(data: schemas.UserDataUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
    ...
    if data.birthdate is not None: current_user.birthdate = data.birthdate
    if data.goal is not None: current_user.goal = data.goal
    if data.photo_url is not None: current_user.photo_url = data.photo_url
    db.commit()
...
def upgrade_subscription(db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
...
def delete_user_account(db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
