...
def decrement_inventory_item(item_name: str, db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
    ...
    if db_item.quantity > 1:
        ...
        db.commit()
        db.refresh(db_item)
        return {"name": db_item.name, "quantity": db_item.quantity, "unit": db_item.unit, "id": db_item.id, "owner_id": db_item.owner_id}
    else:
        db.delete(db_item)
        db.commit()
...
def suggest_shopping_list(db: Session = Depends(get_db), current_user: models.User = Depends(security.get_current_user)):
    if client is None:
